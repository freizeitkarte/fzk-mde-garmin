...
#  [ 6124, 'Freizeitkarte_CAN',                    'http://download.geofabrik.de/north-america/canada-latest.osm.pbf',                                  'CAN',                      'en', 'no_old_name',                             3, 'NA'             ],
  ...
  [ 6484, 'Freizeitkarte_MEX',                    'https://download.geofabrik.de/north-america/mexico-latest.osm.pbf',                                 'MEX',                      'en', 'no_old_name',                             3, 'NA'             ],
...
sub create_licensefile_nsis {
  ...
  # 2021-04-12/pab: forcing UTF-8, newer nsis is having troubles if not UTF-8
  #open ( my $fh_de, '+>', $filename_de ) or die ( "Can't open $filename_de: $OS_ERROR\n" );
  open ( my $fh_de, '+>:encoding(UTF-8)', $filename_de ) or die ( "Can't open $filename_de: $OS_ERROR\n" );
  binmode( $fh_de, ":encoding(UTF-8)");

  printf { *STDOUT } ( "Creating $filename_en  ...\n" );
  # 2021-04-12/pab: forcing UTF-8, newer nsis is having troubles if not UTF-8
	#open ( my $fh_en, '+>', $filename_en ) or die ( "Can't open $filename_en: $OS_ERROR\n" );
	open ( my $fh_en, '+>:encoding(UTF-8)', $filename_en ) or die ( "Can't open $filename_en: $OS_ERROR\n" );
  binmode( $fh_en, ":encoding(UTF-8)");
