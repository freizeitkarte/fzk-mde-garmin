...
my %lic_fzk = (
   ...
   'additional_info_de'     => encode('utf8', decode('iso-8859-1',"Die hier verf�gbaren Karten stellen ein aus den Karten- und H�hendaten abgeleitetes Werk (produced work) dar. Die Karten k�nnen f�r private oder wissenschaftliche Zwecke frei (uneingeschr�nkt) genutzt werden.\n")),
   ...
   'use_de'                 => encode('utf8', decode('iso-8859-1',"Nutzung des Kartenmaterial:\nDie Nutzung des Kartenmaterials erfolgt auf eigene Gefahr. Das Kartenmaterial und oder das Routing kann Fehler enthalten oder unzureichend sein. Die Ersteller dieser Karten �bernehmen keinerlei Gew�hrleistung oder Haftung f�r Sch�den die direkt oder indirekt durch die Nutzung des Kartenmaterial entstehen.\n")),
...
sub fetch_osmdata {
  ...
  # auf g�ltige osm.pbf-Datei pr�fen
...
#  my $filename_en = "$WORKDIRLANG/$mapname.nsis.license.en";
  ...
  printf { $fh_de }
    (   encode('utf8', decode('iso-8859-1',"Lizenzbedingungen der H�henlinien:\n"))
      . "Lizenztyp: %s\n"
      . "Von: %s\n"
      . "Name: %s\n"
      . "Webseite: %s\n"
...
#      . "x-housenumbers\n" );
  ...
  # -- no more options after that line / hier keine Optionen anf�gen --
...
# -----------------------------------------
# Garmin-Map-File f�r BaseCamp erzeugen.
...
sub create_nsis_nsifile {
  ...
  printf { $fh } ( "; - Eintragen der Windows-Registry-Keys f�r die Kartennutzung\n" );
  printf { $fh } ( "; - Eintragen der Windows-Registry-Keys f�r die Deinstallation\n" );
  ...
  printf { $fh } ( "\n" );
  printf { $fh } ( "; Name des Windows-Registrierungsschl�ssels\n" );
  printf { $fh } ( "!define REG_KEY \"%s\"\n",     $mapname );
  printf { $fh } ( "\n" );
  printf { $fh } ( "; Name des alten Windows-Registrierungsschl�ssels (vor Umbenennung der Karten)\n" );
